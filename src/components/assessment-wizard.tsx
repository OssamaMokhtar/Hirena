...
import {
  ...
  PM_AI_INFERENCE_QUESTIONS,
  SOFTWARE_ENGINEERING_AI_INFERENCE_QUESTIONS,
...
export function AssessmentWizard({ onComplete }: AssessmentWizardProps) {
  ...
  const currentSkills = React.useMemo(() => {
    return state.goal.targetTrack === "product-management" ? PM_SKILLS : SOFTWARE_ENGINEERING_SKILLS;
  }, [state.goal.targetTrack]);
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-3">
              ...
              <Select
                ...
                onValueChange={(v) => {
                  updateGoal("targetTrack", v);
                  // Reset target role when track changes
                  setState((prev) => ({
                    ...prev,
                    goal: { ...prev.goal, targetRole: "" },
                    assessment: {},
                    aiInferenceInputs: [],
                  }));
                }}
              ...
              <p className="text-xs text-foreground-muted">
                {state.goal.targetTrack === "product-management"
                  ? "Focus on product strategy, discovery, and delivery."
                  : "Focus on software development, architecture, and engineering practices."}
              </p>
            </div>
              ...
              {benchmarks && (
                <div className="mt-2 rounded-lg bg-surface p-3">
                  <div className="text-sm text-foreground-muted">
                    <span className="font-medium text-foreground">{targetRole}</span> — Median Regional Score:{" "}
                    <span className="font-semibold text-foreground">{benchmarks.medianScore?.toFixed(0)}</span> / 100
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Region */}
            <div className="space-y-2">
              <label className="text-sm font-medium text-foreground">Assessment Region</label>
              <Select
    ...
    return (
      <div className="min-h-screen bg-background">
        <div className="mx-auto max-w-3xl px-4 py-12">
          ...
          <div className="space-y-6">
            {(() => {
              const skillsByCategory: Record<string, Array<{ skillId: string; skill: Skill }>> = {};
              for (const [skillId, skill] of Object.entries(currentSkills)) {
                const cat = skill.category;
                if (!skillsByCategory[cat]) skillsByCategory[cat] = [];
                skillsByCategory[cat].push({ skillId, skill });
              }
                      ...
                      return (
                        <div key={skillId} className="space-y-2 rounded-lg bg-background p-3">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              <label htmlFor={`skill-${skillId}`} className="text-sm font-medium text-foreground">
                                {skill.name}
                              </label>
                              <p className="text-xs text-foreground-muted mt-0.5 line-clamp-2" title={skill.description}>
                                {skill.description}
                              </p>
                            </div>
                          </div>
                          ...
                          <p className="text-xs text-foreground-muted text-right">
                            {currentLevel === 0 ? "Not assessed" : `${currentLevel}/5`}
                          </p>
                        </div>
                      );
                    })}
    ...
    return (
      <div className="min-h-screen bg-background">
        <div className="mx-auto max-w-3xl px-4 py-12">
          ...
          <div className="space-y-6">
            {(state.goal.targetTrack === "product-management" ? PM_AI_INFERENCE_QUESTIONS : SOFTWARE_ENGINEERING_AI_INFERENCE_QUESTIONS).map((item, index) => (
          ...
          <div className="mt-8 flex justify-between">
            ...
            <Button
              onClick={nextStep}
              disabled={state.aiInferenceInputs.filter(i => i.description.trim()).length === 0}
            >
              Continue to Review
            </Button>
          </div>
            ...
            <div className="rounded-lg border border-border bg-surface p-5">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className="mt-1 text-lg font-semibold text-foreground">
                    {state.aiInferenceInputs.filter(i => i.description.trim()).length} / 5 inputs
                  </div>
                </div>
              </div>
            </div>
